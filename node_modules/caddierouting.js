...
router.get("/checkout.html",function(req,resp){
	...
	if(typeof req.user == 'undefined'){
		...
		resp.render("checkout.html.twig",{"liste_commande":req.session.caddy,"total":tot,"nbitem":nbitem});
	}else{
		...
		mysqlhelper.pool.query('SELECT orders.idcustomer, orders.deleted, orders.id, orders.codeorder, orders.idproduct, orders.quantity, orders.statusdate, orders.status, orders.price, products.label, products.picture FROM orders, products WHERE idcustomer = (SELECT customers.id FROM customers, app_users WHERE app_users.username= ? AND customers.email = app_users.email) AND products.id = orders.idproduct AND orders.status = "en caddie" AND orders.deleted=0 ',[username],function(err,rows,fields){
			if(err != null){
				...
				resp.render("checkout.html.twig",{"username":req.user.username, "liste_commande":rows,"total":tot,"nbitem":nbitem});
			}
		});
	}
});
...
router.get("/caddie/ajouter/:id/:quantite",function(req,resp){
	...
	mysqlhelper.pool.query('SELECT price, picture FROM products WHERE id = ? ',[id],function(err,rows,fields){
		...
		if(typeof req.user == 'undefined'){
			...
			var tot = total(req);
	        resp.status(200).end(JSON.stringify({'total':tot,'nbitem':req.session.caddy.length}));
		}
		else{
			username = req.user.username;
			...
			mysqlhelper.pool.query('INSERT INTO orders (idproduct, quantity, idcustomer, price, statusdate, codeorder, status) VALUES (?, ?, (SELECT customers.id FROM customers, app_users WHERE app_users.username = ? AND app_users.email = customers.email), (SELECT price FROM products WHERE id = ?) * ?, NOW(), (SELECT customers.id FROM customers, app_users WHERE app_users.username = ? AND app_users.email = customers.email), "en caddie") ',[id,quantite,username,id,quantite,username],function(err,rows,fields){
				if(err != null){
					console.log('*** Caddie - ajouter *** SQL query - Error - ***' + err);
				}
				else{
...
router.get("/caddie/supprimer/:id",function(req,resp){
	...
	if(typeof req.user !== 'undefined')
	{
		...
		mysqlhelper.pool.query('INSERT INTO orders_status_history (codeorder, idcustomer, status, statusdate) VALUES ((SELECT orders.codeorder FROM orders, products WHERE products.id = ? AND orders.idproduct = products.id), (SELECT customers.ID FROM customers, app_users WHERE app_users.username = ? AND app_users.email = customers.email), "annulée", NOW()) ',[id,username],function(err,rows,fields){
			if(err != null)
			{
...
//			resp.status(200).end(''+tot);
			...
			mysqlhelper.pool.query('UPDATE orders SET quantity = ?,  price = ? WHERE id = ? ',[fromSession.quantity,fromSession.price,id],function(err,rows,fields){
				if(err != null)
				{
